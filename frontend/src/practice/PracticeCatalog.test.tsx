...
describe('practice catalog', () => {
    ...
    render(
      <MemoryRouter initialEntries={['/practice']}>
        <Routes>
          <Route
            path="/practice"
            element={<PracticeCatalog locale="zh" authenticated={false} userId={null} />}
          />
          <Route path="/practice/:slug" element={<h1>工作台</h1>} />
        </Routes>
      </MemoryRouter>,
    )
  ...
  it('writes filters to the request URL and disables anonymous progress filters', async () => {
    render(
      <MemoryRouter initialEntries={['/practice']}>
        <PracticeCatalog locale="zh" authenticated={false} userId={null} />
      </MemoryRouter>,
    )
    ...
    await waitFor(() =>
      expect(
        vi.mocked(fetch).mock.calls.some(([url]) => String(url).includes('difficulty=easy')),
      ).toBe(true),
    )
  })
  ...
  it('does not refetch when only the display language switches', async () => {
    const { rerender } = render(
      <MemoryRouter initialEntries={['/practice']}>
        <PracticeCatalog locale="zh" authenticated userId={1} />
      </MemoryRouter>,
    )
    await screen.findByText('区间质数统计')
    ...
    rerender(
      <MemoryRouter initialEntries={['/practice']}>
        <PracticeCatalog locale="en" authenticated userId={1} />
      </MemoryRouter>,
    )
    expect(screen.getByText('区间质数统计')).toBeInTheDocument()
  ...
  it('refetches when the signed-in account changes', async () => {
    ...
    rerender(
      <MemoryRouter initialEntries={['/practice']}>
        <PracticeCatalog locale="zh" authenticated userId={2} />
      </MemoryRouter>,
    )
