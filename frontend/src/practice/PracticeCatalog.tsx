...
export function PracticeCatalog({
  ...
  useEffect(() => {
    ...
    apiRequest<PracticeCatalogData>(
      `/practice/exercises${query ? `?${query}` : ''}`,
      {},
      zhRef.current ? '题库加载失败' : 'Failed to load exercises',
      controller.signal,
    )
      .then((value) => {
        setData(value)
    ...
    <div className="practice-catalog-page" data-testid="practice-catalog">
      ...
      <section className="practice-filters" aria-label={zh ? '题库筛选' : 'Problem filters'}>
        <label className="practice-search">
          ...
          <input
            ...
            placeholder={zh ? '搜索题目或知识点' : 'Search problems or concepts'}
          />
        </label>
        <div className="practice-filter-row">
          <SlidersHorizontal size={17} aria-hidden="true" />
          <select
            aria-label={zh ? '课程' : 'Course'}
            value={selectedCourse}
            onChange={(event) => update('course', event.target.value)}
          >
            <option value="">{zh ? '全部课程' : 'All courses'}</option>
            {data.facets.courses.map((course) => (
          ...
          <select
            aria-label={zh ? '课时' : 'Lesson'}
            value={params.get('lesson_id') || ''}
            onChange={(event) => update('lesson_id', event.target.value)}
          >
          ...
          {params.toString() && (
            <button
              className="icon-btn"
              type="button"
              aria-label={zh ? '清除筛选' : 'Clear filters'}
              title={zh ? '清除筛选' : 'Clear filters'}
              onClick={() => {
                setSearch('')
                setParams({})
              }}
      ...
      {state === 'loading' && (
        <div className="practice-state">
          ...
          {zh ? '正在整理题库' : 'Loading practice catalog'}
        </div>
      )}
      {state === 'error' && (
        <div className="practice-state">
          <p>{zh ? '题库暂时无法加载。' : 'The catalog could not be loaded.'}</p>
      ...
      {state === 'success' && data.items.length === 0 && (
        <div className="practice-state">
          ...
          <p>{zh ? '没有符合条件的题目。' : 'No problems match these filters.'}</p>
        </div>
      )}
      {state === 'success' && data.items.length > 0 && (
        <div className="practice-table" aria-label={zh ? '练习题列表' : 'Practice problems'}>
          <div className="practice-table-head" role="row">
            <span>{zh ? '状态' : 'Status'}</span>
            <span>{zh ? '题目' : 'Problem'}</span>
            <span>{zh ? '对应章节' : 'Curriculum'}</span>
            <span>{zh ? '难度' : 'Level'}</span>
          </div>
          {data.items.map((item, index) => (
            <button
              ...
              key={item.slug}
              onClick={() =>
                navigate(`/practice/${item.slug}`, {
                  state: { from: `/practice?${params}` },
                })
              }
            >
              <span
                className={`practice-status ${item.progress?.status || 'not_started'}`}
                title={item.progress?.status || 'not started'}
              >
              ...
              <span className="practice-curriculum">
      ...
      <footer className="practice-pagination">
        <span>
          {zh ? `第 ${data.page} / ${pageCount} 页` : `Page ${data.page} of ${pageCount}`}
        </span>
        <div>
          <button
            className="icon-btn"
