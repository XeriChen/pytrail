...
export function PracticeWorkspace({
  ...
  useEffect(() => {
    ...
    apiRequest<PracticeDetail>(
      ...
      {},
      zhRef.current ? '题目加载失败' : 'Failed to load problem',
      controller.signal,
    )
      .then((value) => {
        setDetail(value)
  ...
  const signature = useMemo(
    () =>
      detail
        ? `${detail.function_name}(${detail.signature.parameters.map((item) => `${item.name}: ${item.type}`).join(', ')}) -> ${detail.signature.returns}`
        : '',
    [detail],
  )
  ...
  const run = async () => {
    if (!authReady) return
    if (!authenticated && !anonymousMode) {
      onAuth()
      return
    }
    ...
    try {
      const nextResult = await apiRequest<PracticeRunResult>(
        `/practice/exercises/${detail.slug}/run`,
  ...
  const resetCode = () => {
    if (
      ...
      !window.confirm(
        zh ? '放弃当前修改并恢复起始代码？' : 'Discard your changes and restore the starter code?',
      )
    )
      return
  ...
  if (state === 'loading')
    return (
      <div className="practice-state workspace-state">
        ...
        {zh ? '正在打开工作台' : 'Opening workspace'}
      </div>
    )
  if (state === 'error' || !detail)
    return (
      <div className="practice-state workspace-state">
        <p>{zh ? '题目暂时无法加载。' : 'This problem could not be loaded.'}</p>
        <button className="secondary-btn" onClick={() => setRequestKey((value) => value + 1)}>
          {zh ? '重试' : 'Retry'}
        </button>
      </div>
    )
    ...
    <div className="practice-workspace" data-testid="practice-workspace">
      <header className="workspace-toolbar">
        <button
          className="icon-btn"
          type="button"
          aria-label={zh ? '返回题库' : 'Back to catalog'}
          onClick={goBack}
        >
          <ArrowLeft size={19} />
        </button>
        <div>
          <small>
            {detail.course.title} / {detail.lesson.title}
          </small>
      ...
      <div className="workspace-mobile-tabs" role="tablist">
        {(['statement', 'code', 'results'] as const).map((tab) => (
          <button
            key={tab}
            role="tab"
      ...
      <div className="workspace-split">
        <section
          className={`workspace-statement ${mobileTab === 'statement' ? 'mobile-active' : ''}`}
        >
          <div className="workspace-heading">
            <p className="eyebrow">{zh ? '函数挑战' : 'FUNCTION CHALLENGE'}</p>
            <h1>{detail.title}</h1>
          ...
          <React.Suspense
            fallback={
              <div className="practice-state">{zh ? '正在渲染题面' : 'Rendering statement'}</div>
            }
          >
            <CourseMarkdown
              markdown={detail.prompt}
              assetBaseUrl=""
              onLessonLink={() => {}}
              theme={theme}
            />
          ...
          <div className="function-contract">
            <Code2 size={18} />
            <div>
              <small>{zh ? '函数签名' : 'Function contract'}</small>
              <code>{signature}</code>
            </div>
          </div>
            ...
            {detail.cases.map((item) => (
              <article key={item.order}>
                ...
                <div>
                  ...
                  <strong>→ {JSON.stringify(item.expected)}</strong>
          ...
          <button
            ...
            type="button"
            onClick={() => onOpenLesson(detail.lesson.id)}
          >
            <BookOpen size={16} />
            {zh ? '回到对应课时' : 'Open related lesson'}
          </button>
        </section>
        <section
        ...
        >
          <div className="editor-toolbar">
            ...
            <div className="editor-toolbar-actions">
              <button
                className="icon-btn"
                type="button"
                aria-label={zh ? '重置代码' : 'Reset code'}
                title={zh ? '重置代码' : 'Reset code'}
                onClick={resetCode}
              >
                <RotateCcw size={16} />
              </button>
          ...
          <CodeMirror
            ...
            theme={theme}
            height="100%"
            minHeight={editorFullscreen ? '0' : '360px'}
            aria-label={zh ? 'Python 代码' : 'Python code'}
            basicSetup={{
              lineNumbers: true,
          ...
          <div className="editor-actions">
            <span>{codeBytes} / 12000 B</span>
            <button
              className="run-practice-btn"
              type="button"
              disabled={!authReady || running || codeBytes > 12000}
              onClick={run}
            >
              <Play size={16} />
              {running ? (zh ? '运行中' : 'Running') : zh ? '运行样例' : 'Run examples'}
            </button>
          </div>
        </section>
        <aside className={`workspace-results ${mobileTab === 'results' ? 'mobile-active' : ''}`}>
          <div className="results-head">
            ...
            {result && (
              <strong className={result.passed ? 'passed' : 'failed'}>
                {result.passed
                  ? zh
                    ? '全部通过'
                    : 'All passed'
          ...
          {!result && (
            <div className="results-empty">
              ...
              <p>
                {zh
                  ? '运行代码后，这里会显示每个公开样例的结果。'
                  : 'Run your code to inspect every public example.'}
              </p>
            </div>
          )}
          {result?.error && (
          ...
          {result?.cases.map((item) => (
            <article
              className={item.passed ? 'result-case passed' : 'result-case failed'}
              key={item.order}
            >
              {item.passed ? <CheckCircle2 size={18} /> : <CircleX size={18} />}
              <div>
                <strong>{zh ? `样例 ${item.order}` : `Case ${item.order}`}</strong>
                {item.error ? (
                  <code>{item.error}</code>
                ) : (
                  <>
                    <small>{zh ? '实际结果' : 'Actual'}</small>
                    <code>{JSON.stringify(item.actual)}</code>
                  </>
                )}
                <span>{item.duration_ms.toFixed(2)} ms</span>
              </div>
            </article>
          ))}
        </aside>
