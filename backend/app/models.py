...
class Lesson(Base):
    ...
    __table_args__ = (Index("ux_lessons_course_source_path", "course_id", "source_path", unique=True),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    course_id: Mapped[int] = mapped_column(ForeignKey("courses.id"))
    source_path: Mapped[str | None] = mapped_column(String(512), nullable=True, index=True)
    title: Mapped[str] = mapped_column(String(160))
...
class Exercise(Base):
    ...
    cases: Mapped[list["ExerciseCase"]] = relationship(
        back_populates="exercise", cascade="all, delete-orphan", order_by="ExerciseCase.order"
    )
