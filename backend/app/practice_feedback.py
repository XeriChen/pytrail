...
FEEDBACK_CATEGORIES = frozenset({"all_passed", "wrong_output", "runtime_error", "validation_error"})


def classify_result(result: dict[str, Any]) -> str:
    """Return a stable, UI-friendly category without exposing a full diagnosis claim."""
    ...
    if any(case.get("error") for case in result.get("cases", []) if isinstance(case, dict)):
