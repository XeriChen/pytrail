...
COURSE_SPECS = (
    ...
    CourseSpec(
        ...
        "Day31-35",
        "语言进阶与 Linux",
        "进阶 Python、Web 前端基础与 Linux 操作系统。",
        "intermediate",
        "gold",
        3,
    ),
    ...
    CourseSpec(
        ...
        "Day46-60",
        "Django Web 开发",
        "用 Django 与 DRF 构建、测试并部署 Web 应用。",
        "intermediate",
        "cinnabar",
        5,
    ),
    CourseSpec(
        ...
        "Day61-65",
        "网络数据采集",
        "掌握网络请求、HTML 解析、并发、Selenium 与 Scrapy。",
        "intermediate",
        "jade",
        6,
    ),
...
FOUNDATION_EXERCISES: tuple[tuple[ExerciseSeed, ...], ...] = (
    (
        _exercise(
            "Which command prints the Python interpreter version?",
            "# Run this in a terminal, not in Python:\n# python --version\n\nprint('Python is ready')",
            "python",
        ),
    ),
    (
        _exercise(
            "Which built-in function writes text to the screen?",
            "print('hello, world')\nprint('goodbye, world')",
            "print",
        ),
    ),
    (
        _exercise(
            "What does `type(3.14).__name__` return?",
    ...
    (
        _exercise(
            "Which keyword introduces an alternative condition after `if`?",
            "score = 85\nif score >= 90:\n    print('A')\nelif score >= 80:\n    print('B')\nelse:\n    print('C')",
            "elif",
        ),
    ),
    (
        _exercise(
            "What is the sum of integers 1 through 100?",
    ...
    (
        _exercise(
            "The 10th Fibonacci number (1, 1, 2, 3, 5, 8, ...) is what?",
            "a, b = 0, 1\nfor _ in range(10):\n    a, b = b, a + b\n    print(a)",
            "55",
        ),
    ),
    (
        _exercise(
            "For `lst = [10, 20, 30, 40]`, what does `lst[-1]` return?",
            "lst = [10, 20, 30, 40]\nprint(lst[-1])\nprint(lst[::2])",
            "40",
        ),
    ),
    ...
    (
        _exercise(
            "What is the type of `(100,)` (note the trailing comma)?",
            "print(type((100,)))\nprint(type((100)))",
            "tuple",
        ),
    ),
    ...
    (
        _exercise(
            "Which method returns a value for a key without raising an error if the key is missing?",
    ...
    (
        ...
        _exercise(
            "What is the factorial of 5 (i.e. `5!`)?", "# 5 * 4 * 3 * 2 * 1 = ?", "120"
        ),
    ),
    (
        _exercise(
    ...
    (
        _exercise(
            "Which `functools` decorator memoizes a function's results?",
    ...
    (
        _exercise(
            "Which function calls the parent class's `__init__` method?",
            "class Person:\n    def __init__(self, name):\n        self.name = name\n\nclass Teacher(Person):\n    def __init__(self, name, course):\n        super().__init__(name)\n        self.course = course\n\nprint(Teacher('Ada', 'Python').name)",
            "super",
        ),
    ),
    (
        _exercise(
            "Which magic method overloads the `<` operator?",
...
def resolve_content_root() -> Path:
    ...
    return (
        Path(__file__).resolve().parents[1] / "content" / "python-100-days"
    ).resolve()
    ...
    for spec in specs:
        if spec.slug in seen_slugs or spec.source_dir in seen_sources:
            raise ContentSyncError(
                f"Duplicate course identity: {spec.slug} / {spec.source_dir}"
            )
        ...
        if not course_dir.is_dir():
            raise ContentSyncError(
                f"Missing course directory for {spec.slug}: {course_dir}"
            )
        resource_dir = course_dir / "res"
        if not resource_dir.is_dir():
            raise ContentSyncError(
                f"Missing resource directory for {spec.slug}: {resource_dir}"
            )
        markdown_paths = sorted(course_dir.glob("*.md"), key=lesson_sort_key)
        if not markdown_paths:
            raise ContentSyncError(
                f"No Markdown lessons found for {spec.slug}: {course_dir}"
            )
        ...
        for sequence, path in enumerate(markdown_paths, start=1):
            try:
                ...
                raise ContentSyncError(
                    f"Invalid UTF-8 Markdown for {spec.slug}: {path}"
                ) from exc
            source_path = f"{spec.source_dir}/{path.name}"
            quick_checks = (
                FOUNDATION_EXERCISES[sequence - 1]
                if spec.slug == "python-foundations"
                and sequence <= len(FOUNDATION_EXERCISES)
                else ()
            )
            exercises = tuple(
        ...
        manifests.append(
            CourseManifest(
                spec, tuple(records), _digest_files(asset_files, resource_dir)
            )
        )
    ...
    if resolved_practice_root is not None:
        ...
        for manifest in manifests:
            ...
            for seed in programming[manifest.spec.slug]:
                by_source.setdefault(seed.lesson_source_path, []).append(
                    _programming_seed(seed)
                )
            lessons = tuple(
                replace(
                    record,
                    exercises=record.exercises
                    + tuple(by_source.get(record.source_path, ())),
                )
...
def _exercise_seed_state(seed: ExerciseSeed, order: int) -> tuple[object, ...]:
    return (
        ...
        tuple(
            _case_seed_state(case, position)
...
def _exercise_row_state(row: Exercise) -> tuple[object, ...]:
    return (
        ...
        tuple(
            _case_row_state(case)
            for case in sorted(row.cases, key=lambda item: item.order)
        ),
    )
...
def _lesson_links(
    markdown: str, source_path: str, ids_by_source: dict[str, int]
) -> dict[str, int]:
    links: dict[str, int] = {}
...
def content_index_from_db(
    ...
    for manifest in manifests:
        ...
        if course is None:
            raise ContentSyncError(
                f"Course missing after synchronization: {manifest.spec.slug}"
            )
        ...
        for record in manifest.lessons:
            ...
            if lesson is None:
                raise ContentSyncError(
                    f"Lesson missing after synchronization: {record.source_path}"
                )
    ...
    return ContentIndex(
        content_root=root,
        course_order={
            manifest.spec.slug: manifest.spec.order for manifest in manifests
        },
        course_assets={
            manifest.spec.slug: root / manifest.spec.slug / "res"
            for manifest in manifests
        },
