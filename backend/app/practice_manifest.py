...
def _read_document(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise PracticeManifestError(f"Invalid JSON in practice manifest {path}: {exc}") from exc
...
def _parse_signature(value: Any, context: str) -> FunctionSignatureSeed:
    ...
    if not isinstance(raw_parameters, list) or len(raw_parameters) > 8:
        raise PracticeManifestError(f"{context}: signature.parameters must contain at most 8 items")
    ...
    for position, raw in enumerate(raw_parameters, start=1):
        ...
        if not name.isidentifier() or keyword.iskeyword(name) or name.startswith("_"):
            raise PracticeManifestError(f"{item_context}: invalid parameter name {name!r}")
        if name in names:
...
def _parse_case(value: Any, signature: FunctionSignatureSeed, context: str) -> PracticeCaseSeed:
    if not isinstance(value, dict):
    ...
    if not isinstance(raw_args, list) or not isinstance(raw_kwargs, dict):
        raise PracticeManifestError(f"{context}: args must be an array and kwargs must be an object")
    names = [parameter.name for parameter in signature.parameters]
    if len(raw_args) > len(names):
        raise PracticeManifestError(f"{context}: arguments do not match the function signature")
    remaining = set(names[len(raw_args):])
    if set(raw_kwargs) != remaining:
        raise PracticeManifestError(f"{context}: arguments do not match the function signature")
    try:
        payload_size = len(json.dumps(value, ensure_ascii=False, allow_nan=False).encode("utf-8"))
    except (TypeError, ValueError) as exc:
        raise PracticeManifestError(f"{context}: case values must be JSON-safe") from exc
    ...
    if not isinstance(explanation, str) or len(explanation) > 2_000:
        raise PracticeManifestError(f"{context}: explanation must be a string up to 2000 characters")
    ...
    if isinstance(tolerance, bool) or not isinstance(tolerance, (int, float)) or not 0 < tolerance <= 1:
        raise PracticeManifestError(f"{context}: tolerance must be greater than 0 and at most 1")
...
def _parse_exercise(
    ...
    if not SLUG_RE.fullmatch(slug):
        raise PracticeManifestError(f"{context}: slug must use lowercase words separated by hyphens")
    lesson_source_path = _required_text(value.get("lesson_source_path"), "lesson_source_path", context, 512)
    ...
    function_name = _required_text(value.get("function_name"), "function_name", context, 80)
    if not function_name.isidentifier() or keyword.iskeyword(function_name) or function_name.startswith("_"):
        raise PracticeManifestError(f"{context}: invalid function_name {function_name!r}")
...
def load_practice_manifests(
    ...
    for course_slug in expected:
        ...
        if not isinstance(raw_exercises, list) or len(raw_exercises) != 4:
            raise PracticeManifestError(f"{path}: each course must contain exactly 4 exercises")
        if course_slug not in lesson_sources:
            raise PracticeManifestError(f"{path}: no discovered lessons for course {course_slug}")
        exercises = tuple(
