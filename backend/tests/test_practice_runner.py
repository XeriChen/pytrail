...
from app.practice_runner import PracticeCaseInput, PracticeRunError, run_practice, validate_source
...
"""
    ...
    def test_rejects_unsafe_syntax_and_wrong_signatures(self) -> None:
        ...
        for source, message in blocked.items():
            with self.subTest(source=source), self.assertRaisesRegex(PracticeRunError, message):
                validate_source(source, "add", ["left", "right"])

    def test_timeout_is_fail_closed(self) -> None:
        result = run_practice("def add(left, right):\n    while True: pass\n", "add", ["left", "right"], CASES, timeout=0.1)
